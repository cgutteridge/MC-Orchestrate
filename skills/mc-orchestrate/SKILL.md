...
- Placement-then-build planner: `/Users/cjg/Projects/MC-Orchestrate/src/planner/aiPlanner.ts` (`runPlacementThenBuild`)
...
## AI planning (two steps)

Every chat build runs **placement then build**: (1) get the location and size, (2) emit the layer-map plan. The orchestrator calls `runPlacementThenBuild()` up to `MCORCH_AI_PLAN_MAX_STEPS` AI calls per request (default **10**; includes retries).

1. **Location** — placement-phase context (player position, look vector, target block, 15×8×15 world scan, optional **CONVERSATION HISTORY** + **LAST BUILD SUMMARY** lines, JSON including `lastBuiltStructureSummary` and full `lastBuiltStructure`).
2. **Build** — `{ action: "placement_choice", placement }` locks placement and size; then `build` with a plan only (placement merged server-side).
3. The orchestrator repairs the plan (`repairLoosePlanCandidate`), validates it via `PlanSchema`, then executes.
4. After execution, the orchestrator can optionally run `runVerifyPass()` for a polish pass (disk reads via `WorldReader` when verify extends beyond placed blocks).
...
- What action did the AI return on each call (`placement_choice`, `build`, or bare plan)?
- Did placement → plan transition correctly between steps?
