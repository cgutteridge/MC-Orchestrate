...
- By default the AI must emit **`layerMap`** passes (`primitives: []`); primitive ops (`fill_cuboid`, etc.) are disabled unless `MCORCH_LAYER_MAP_ONLY=false` (Vitest sets this for unit tests). See `src/planner/planMode.ts`. Intended shape: **one pass = full structure** in a single `layers` array; **optional second pass** = refinement only — at most two layer-map passes (`validatePlanSemantics` in `src/planner/semantics.ts`).
