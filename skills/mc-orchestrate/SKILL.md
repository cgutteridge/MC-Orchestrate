---
name: mc-orchestrate
description: Use when working in the MC-Orchestrate repository to build, debug, review, or extend the Minecraft plugin, HTTP orchestrator, planner, bridge, and live local server workflow. Covers planner/schema alignment, bridge-only world mutation, runtime restart procedure, log inspection, and backlog/skill updates when concrete new repo-specific issues are confirmed.
---
...
- Treat the stack as: Spigot plugin -> HTTP orchestrator -> planner -> bridge -> Minecraft server.
- Prefer fixing concrete behavioral defects over broad refactors.
- Keep [TASKS.md](/Users/cjg/Projects/MC-Orchestrate/TASKS.md) aligned with newly confirmed issues.
...
- AI planner: `/Users/cjg/Projects/MC-Orchestrate/src/planner/aiPlanner.ts`
- Heuristic planner: `/Users/cjg/Projects/MC-Orchestrate/src/planner/heuristicPlanner.ts`
...
- Backlog: `/Users/cjg/Projects/MC-Orchestrate/TASKS.md`
...
- Planner stages: `/Users/cjg/Projects/MC-Orchestrate/logs/ai-planner.jsonl`
- Raw provider trace: `/Users/cjg/Projects/MC-Orchestrate/logs/ai-provider.log`

Use them to answer:

- What did the model actually return?
- Did JSON extraction/parsing change the meaning?
- Did repair logic drop or invent primitives?
...
- Keep the prompt schema and executor schema exactly aligned.
- Do not let unsupported requests get coerced into the nearest supported intent such as `build_house`.
...
- Update [TASKS.md](/Users/cjg/Projects/MC-Orchestrate/TASKS.md) when new confirmed issues appear.
